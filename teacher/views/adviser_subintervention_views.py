...
def get_intervention_students(request):
    ...
    for student in students:
        ...
        # If there's an intervention, get the latest action
        if intervention:
            latest_action = InterventionAction.objects.filter(
                intervention_plan=intervention
            ).order_by('-id').first()  # Fetch the latest action
            
            if latest_action:
                intervention_tier = latest_action.tier  # Get tier from the latest action
            else:
                intervention_tier = intervention.current_tier  # Fallback
        else:
            intervention_tier = 'None'  # If no intervention exists
            
            # Existing logic to determine if a new intervention is needed...
        
        # Build student data with the correct intervention tier
        student_data = {
            'id': student.id,
            'lrn': student.lrn,
            'name': f"{student.last_name}, {student.first_name} {student.middle_name}".strip(),
            'sex': student.gender,
            'age': student.age,
            'current_grade': intervention.current_grade if intervention else 0,
            'absences': intervention.total_absences if intervention else 0,
            'missing_work': intervention.missing_written_works + intervention.missing_performance_tasks if intervention else 0,
            'missing_ww': intervention.missing_written_works if intervention else 0,
            'missing_pt': intervention.missing_performance_tasks if intervention else 0,
            'missed_qa': intervention.missed_quarterly_assessment if intervention else False,
            'risk_level': intervention.risk_level if intervention else 'On Track',
            'intervention_tier': intervention_tier,  # Updated to use the latest action's tier
            'intervention_id': intervention.id if intervention else None,
            'has_intervention': True if intervention else False,
            'is_resolved': intervention.is_resolved if intervention else False,
        }
...
def get_student_intervention_details(request, student_id):
    ...
    # Determine the latest action to get the correct tier
    latest_action = actions.first()  # Get the latest action
    ...
    data = {
        ...
        'intervention': {
            'id': intervention.id,
            'risk_level': intervention.risk_level,
            'current_tier': intervention_tier,  # Use the tier from the latest action
            'current_grade': intervention.current_grade,
            'total_absences': intervention.total_absences,
            'missing_ww': intervention.missing_written_works,
            'missing_pt': intervention.missing_performance_tasks,
