...
#     # NEW: __init__ for admin editability (makes disabled/readonly fields editable for staff)
#     def __init__(self, *args, user=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         if user and user.is_staff:
#             # Admin: Remove readonly/disabled from ALL fields (e.g., sped_details, working_details, lrn editable)
#             for field in self.fields.values():
#                 field.widget.attrs.pop('readonly', None)
#                 field.widget.attrs.pop('disabled', None)
#                 # Add admin highlight class
#                 current_class = field.widget.attrs.get('class', '')
#                 field.widget.attrs['class'] = f"{current_class} admin-editable".strip() if current_class else 'admin-editable'
                
#         else:
#             # Non-admin: Enforce readonly on sensitive fields (e.g., lrn, details disabled)
#             self.fields['lrn'].widget.attrs['readonly'] = True
#             self.fields['sped_details'].widget.attrs['disabled'] = True
#             self.fields['working_details'].widget.attrs['disabled'] = True
...
#     # NEW: __init__ for admin editability (no explicit readonly, but enables all + enforces sensitive for non-admin)
#     def __init__(self, *args, user=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         if user and user.is_staff:
#             # Admin: Remove any readonly/disabled (full edit, e.g., guardian details)
#             for field in self.fields.values():
#                 field.widget.attrs.pop('readonly', None)
#                 field.widget.attrs.pop('disabled', None)
#                 # Add admin highlight class
...
#                 field.widget.attrs['class'] = f"{current_class} admin-editable".strip() if current_class else 'admin-editable'
#         else:
#             # Non-admin: Enforce readonly on sensitive (e.g., guardian_relationship, parent_photo disabled)
#             self.fields['guardian_relationship'].widget.attrs['readonly'] = True
#             self.fields['parent_photo'].widget.attrs['disabled'] = True
#             # Add more if needed, e.g., self.fields['father_email'].widget.attrs['readonly'] = True
...
#     # NEW: __init__ for admin editability (enables all; enforces disabled on sensitive like marital_status for non-admin)
#     def __init__(self, *args, user=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         if user and user.is_staff:
#             # Admin: Remove readonly/disabled from ALL fields (e.g., marital_status, highest_education editable)
#             for field in self.fields.values():
#                 field.widget.attrs.pop('readonly', None)
#                 field.widget.attrs.pop('disabled', None)
#                 # Add admin highlight class
...
#                 field.widget.attrs['class'] = f"{current_class} admin-editable".strip() if current_class else 'admin-editable'
#         else:
#             # Non-admin: Enforce readonly/disabled on sensitive fields (e.g., marital_status, highest_education)
#             self.fields['marital_status'].widget.attrs['disabled'] = True
#             self.fields['highest_education'].widget.attrs['disabled'] = True
#             # Add more if needed, e.g., self.fields['confidence_level'].widget.attrs['disabled'] = True
...
#         ]
#         widgets = {
#             'lrn': forms.TextInput(attrs={'placeholder': 'Enter LRN (numbers only)', 'readonly': True}),  # Added readonly for non-admin
#             'dost_exam_result': forms.Select(choices=[('', '-- Select Result --'), ('passed', 'Passed'), ('failed', 'Failed'), ('not-taken', 'Not Taken')]),
#             'report_card': forms.FileInput(),
#             'mathematics': forms.NumberInput(attrs={'placeholder': 'Grade', 'min': 75, 'max': 100, 'step': 0.01}),
#             'araling_panlipunan': forms.NumberInput(attrs={'placeholder': 'Grade', 'min': 75, 'max': 100, 'step': 0.01}),
...
#     # NEW: __init__ for admin editability (removes readonly on lrn, disables grades for non-admin)
#     def __init__(self, *args, user=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         if user and user.is_staff:
#             # Admin: Remove readonly from lrn; enable all (e.g., edit grades)
...
#                 field.widget.attrs['class'] = f"{current_class} admin-editable".strip() if current_class else 'admin-editable'
#         else:
#             # Non-admin: Enforce readonly on lrn; disable grades
#             self.fields['lrn'].widget.attrs['readonly'] = True
...
#         fields = ['status', 'selected_program']
#         widgets = {
#             'status': forms.Select(choices=[('pending', 'Pending'), ('approved', 'Approved'), ('rejected', 'Rejected')], attrs={'disabled': True}),  # Added disabled for non-admin
#             'selected_program': forms.Select(choices=[
#                 ('ste', 'STE'), ('spfl', 'SPFL'), ('sptve', 'SPTVE'), 
#                 ('top5', 'TOP 5'), ('hetero', 'HETERO'), ('ohsp', 'OHSP'), ('regular', 'Regular')
#             ], attrs={'readonly': True}),  # Added readonly for non-admin
#             'placement_date': forms.DateInput(attrs={'type': 'date'}),
#         }

#     # NEW: __init__ for admin editability (enables status/program for staff)
#     def __init__(self, *args, user=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         if user and user.is_staff:
#             # Admin: Remove disabled/readonly (e.g., change status to 'Approved')
#             self.fields['status'].widget.attrs.pop('disabled', None)
#             self.fields['selected_program'].widget.attrs.pop('readonly', None)
#             for field in self.fields.values():
#                 # Add admin highlight class
#                 current_class = field.widget.attrs.get('class', '')
#                 field.widget.attrs['class'] = f"{current_class} admin-editable".strip() if current_class else 'admin-editable'
#         else:
#             # Non-admin: Enforce disabled/readonly (can't change placement)
