...
document.addEventListener("DOMContentLoaded", () => {
  if (window.teachersData && Array.isArray(window.teachersData)) {
    ...
    console.error("No teachers data found in window.teachersData");
  }
});
...
function populateTeachersTable() {
  ...
  teachersData.forEach((teacher) => {
    const fullName = teacher.fullName || `${teacher.firstName} ${teacher.middleName || ""} ${teacher.lastName}`.trim();
    const row = document.createElement("tr");
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="preview-btn" onclick="viewTeacherProfile(${teacher.id})">
          Preview
        </button>
      </td>
    `;
...
function viewTeacherProfile(teacherId) {
  ...
  if (teacher) {
    ...
    document.getElementById('firstName').textContent = teacher.firstName;
    document.getElementById('middleName').textContent = teacher.middleName;
    document.getElementById('gender').textContent = teacher.sex;
    document.getElementById('dateOfBirth').textContent = teacher.dateOfBirth;
    document.getElementById('age').textContent = teacher.age;
    document.getElementById('position').textContent = teacher.position;
    document.getElementById('department').textContent = teacher.department;
    ...
    const teachingLoadBody = document.getElementById('teachingLoadBody');
    teachingLoadBody.innerHTML = '<tr><td colspan="3">Teaching load not available.</td></tr>';  // Placeholder
    
    const changeHistoryBody = document.getElementById('changeHistoryBody');
    changeHistoryBody.innerHTML = '<tr><td colspan="3">No change history.</td></tr>';  // Placeholder
    
    document.getElementById('teacherProfileModal').style.display = 'block';
  } else {
...
function exportToPdf() {
  ...
  const doc = new jsPDF();
  const table = document.getElementById("teachersTable");
  doc.text("ZNHS West - Teachers List", 14, 15);
